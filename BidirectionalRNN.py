...
class BidirectionalRNN:
    def __init__(self, name, lstm_size, data_type=tf.float32):
        ...
        with tf.variable_scope(self.name):
            self.forward_lstm = rnn.MultiRNNCell([rnn.BasicLSTMCell(lstm_size[i]) for i in range(len(lstm_size))])
            self.backward_lstm = rnn.MultiRNNCell([rnn.BasicLSTMCell(lstm_size[i]) for i in range(len(lstm_size))])
